...
void load_elf(const char* fn, state_t *ms) {
  ...
  if(strtabidx && symtabidx) {
    ...
    for(int32_t i = 0; i < (sh32[symtabidx].sh_size / sizeof(Elf64_Sym)); i++) {
      symtab[strtab + sym[i].st_name] = static_cast<uint32_t>(sym[i].st_value);
    }
  }
  munmap(buf, s.st_size);

  if(symtab.find("tohost") != symtab.end()) {
    global::tohost_addr = symtab.at("tohost");
  }
  if(symtab.find("fromhost") != symtab.end()) {
    global::fromhost_addr = symtab.at("fromhost");
  }

...
void execRiscv(state_t *s) {
  ...
  if(s->priv == priv_machine) {
    tohost = *reinterpret_cast<uint64_t*>(mem + global::tohost_addr);
    if(tohost) {
      if(not(global::syscall_emu)) {
	...
	if(cmd == 1) {
	  ...
	  *reinterpret_cast<uint64_t*>(mem + global::tohost_addr) = 0;
	  *reinterpret_cast<uint64_t*>(mem + global::fromhost_addr) = (dev << 56) | (cmd << 48);
	}
	else {
	  abort();
	}
